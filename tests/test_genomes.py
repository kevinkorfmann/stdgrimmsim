...
class TestContig(object):
    ...
    example_dfe = stdgrimmsim.DFE(
        id="abc",
        description="example DFE",
        long_description="test test beep boop beep",
        proportions=[0.3, 0.7],
        mutation_types=[stdgrimmsim.MutationType() for _ in range(2)],
    )
    ...
    def test_default_gc(self):
        contig = stdgrimmsim.Contig.basic_contig(length=42)
    ...
    def test_gc_errors(self):
        # should not error
        contig = stdgrimmsim.Contig.basic_contig(
        ...
        with pytest.raises(ValueError, match="Cannot set.*bacterial recomb"):
            _ = stdgrimmsim.Contig.basic_contig(
                length=100,
        ...
        with pytest.raises(ValueError, match="Must set.*bacterial recomb"):
            _ = stdgrimmsim.Contig.basic_contig(length=100, bacterial_recombination=True)
        # must set gc length and fraction together otherwise
        with pytest.raises(ValueError, match="without setting"):
            _ = stdgrimmsim.Contig.basic_contig(length=100, gene_conversion_fraction=0.4)
        with pytest.raises(ValueError, match="without setting"):
            _ = stdgrimmsim.Contig.basic_contig(length=100, gene_conversion_length=14)
    ...
    def test_gc_length_error(self):
        sp = stdgrimmsim.get_species("StrAga")
    ...
    def test_add_dfe(self):
        for clear in (True, False):
            ...
            if clear:
                contig.clear_dfes()
            props = [0.3, 0.7]
            mt = [stdgrimmsim.MutationType() for _ in props]
            dfes = [
                stdgrimmsim.DFE(
                    id=str(j),
                    description="test",
    ...
    def test_add_dfe_interval_formats(self):
        ...
        for intervals in (
            ...
            contig = stdgrimmsim.Contig.basic_contig(length=L)
        ...
        with pytest.raises(ValueError, match="must be a numpy array"):
            stdgrimmsim.utils._check_intervals_validity([[0, 1]])
    ...
    def test_generic_contig_origin(self):
        length = 42
        contig = stdgrimmsim.Contig.basic_contig(length=length)
        chromosome, left, right = contig.coordinates
    ...
    def test_original_coordinates(self):
        ...
        with pytest.warns(
            stdgrimmsim.DeprecatedFeatureWarning, match="no longer shifted"
        ):
    ...
    def test_dfe_breakpoints_coordinate_system(self):
        ...
        with pytest.warns(
            stdgrimmsim.DeprecatedFeatureWarning, match="relative_coordinates argument"
        ):
    ...
    def test_basic_contig_is_diploid(self):
        contig = stdgrimmsim.Contig.basic_contig(length=1000)
...
class TestGeneConversion(object):
    def test_mean_gene_conversion(self):
        dro_mel = stdgrimmsim.get_species("DroMel")
    ...
    def test_no_mean_gene_conversion(self):
        ana_pla = stdgrimmsim.get_species("AnaPla")
    ...
    def test_modifiying_gene_conversion(self):
        esc_col = stdgrimmsim.get_species("EscCol")
    ...
    def test_use_species_gene_conversion_unnamed_contig_undefined_gc(self):
        species = stdgrimmsim.get_species("AnaPla")
