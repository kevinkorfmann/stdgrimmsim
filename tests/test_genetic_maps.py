...
def setup_module():
    ...
    genetic_maps = list(stdgrimmsim.all_genetic_maps())
    if not genetic_maps:
        return
...
class GeneticMapTestClass(stdgrimmsim.GeneticMap):
    """
    A genetic map that we can instantiate to get a genetic map for testing.
    """

    def __init__(self):
        genome = stdgrimmsim.Genome(chromosomes=[])
...
class TestGeneticMap(tests.CacheWritingTest):
    ...
    def test_cache_dirs(self):
        ...
        cache_dir = stdgrimmsim.get_cache_dir() / "genetic_maps" / gm.species.id / gm.id
...
class TestGeneticMapDownload(tests.CacheWritingTest):
    ...
    def test_correct_url(self):
        gm = GeneticMapTestClass()
        with mock.patch("stdgrimmsim.utils.download", autospec=True) as mocked_get:
...
class TestAllGeneticMaps(tests.CacheReadingTest):
    ...
    def test_non_empty(self):
        ...
        assert isinstance(list(stdgrimmsim.all_genetic_maps()), list)

    def test_types(self):
        for gm in stdgrimmsim.all_genetic_maps():
            assert isinstance(gm, stdgrimmsim.GeneticMap)
...
class TestGetChromosomeMap(tests.CacheReadingTest):
    ...
    def test_known_chromosome(self):
        species = stdgrimmsim.get_species("CanFam")
