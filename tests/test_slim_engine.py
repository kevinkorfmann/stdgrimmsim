...
def count_mut_types(ts):
    selection_coeffs = [
        stdgrimmsim.selection_coeff_from_mutation(ts, mut) for mut in ts.mutations()
    ]
...
class TestAPI:
    ...
    def test_assert_min_version(self):
        engine = stdgrimmsim.get_engine("slim")
        with mock.patch(
            "stdgrimmsim.slim_engine._SLiMEngine.get_version", return_value="3.4"
        ):
        ...
        with mock.patch(
            "stdgrimmsim.slim_engine._SLiMEngine.get_version", return_value="4.0"
        ):
            engine._assert_min_version("3.5", engine.slim_path())
...
class TestCLI:
    ...
    def test_dfe_no_demography(self, tmp_path):
        ...
        cmd = (
            ...
            f"ZweBerg -c 1 -l 0.02 -o {fname} --dfe Gamma_K17 -s 24 "
            f"BlackForest:5"
        ).split()
        capture_output(stdgrimmsim.cli.stdgrimmsim_main, cmd)
        ts = tskit.load(fname)
    ...
    def test_dfe_no_interval(self, tmp_path):
        ...
        cmd = (
            f"-q -e slim --slim-scaling-factor 20 --slim-path {slim_path} "
            f"ZweBerg -c 1 -l 0.01 -o {fname} --dfe Gamma_K17 -s 984 "
            f"BlackForest:5"
        ).split()
        capture_output(stdgrimmsim.cli.stdgrimmsim_main, cmd)
        ts = tskit.load(fname)
    ...
    def test_dfe_interval(self, tmp_path):
        ...
        cmd = (
            f"-q -e slim --slim-scaling-factor 20 --slim-path {slim_path} "
            f"ZweBerg -c 1 -l 0.01 -o {fname} --dfe Gamma_K17 -s 984 "
            f"--dfe-interval 1000,100000 BlackForest:5"
        ).split()
        capture_output(stdgrimmsim.cli.stdgrimmsim_main, cmd)
        ts = tskit.load(fname)
        self.verify_slim_sim(ts, num_samples=10)
    ...
    def test_dfe_demography(self, tmp_path):
        ...
        cmd = (
            f"-q -e slim --slim-scaling-factor 20 --slim-path {slim_path} "
            f"ZweBerg -c 1 -l 0.01 -o {fname} "
            "-d BlackForest_1D12 --dfe Gamma_K17 -s 148 "
    ...
    def test_dfe_annotation(self, tmp_path):
        ...
        cmd = (
            f"-q -e slim --slim-scaling-factor 20 --slim-path {slim_path} "
            f"ZweBerg -c 1 -o {fname} --dfe Gamma_K17 -s 913 "
            "--dfe-annotation ensembl_havana_104_CDS BlackForest:5"
        ).split()
        capture_output(stdgrimmsim.cli.stdgrimmsim_main, cmd)
        ts = tskit.load(fname)
        self.verify_slim_sim(ts, num_samples=10)
    ...
    def test_dfe_bed_file(self, tmp_path):
        lines = [
            "\t".join(["1", "100000", "145000"]),
            "\t".join(["1", "150000", "302425"]),
        ]
        ...
        cmd = (
            f"-q -e slim --slim-scaling-factor 20 --slim-path {slim_path} "
            f"ZweBerg -c 1 -s 1234 -l 0.01 -o {fname} --dfe Gamma_K17 -s 183 "
            f"--dfe-bed-file {tmp_path / 'ex.bed'} BlackForest:5"
        ).split()
        capture_output(stdgrimmsim.cli.stdgrimmsim_main, cmd)
        ts = tskit.load(fname)
        self.verify_slim_sim(ts, num_samples=10)
    ...
    def test_chromosomal_segment(self, tmp_path):
        ...
        cmd = (
            ...
            f"--dfe Gamma_K17 -s 24 BlackForest:5"
        ).split()
        capture_output(stdgrimmsim.cli.stdgrimmsim_main, cmd)
        ts = tskit.load(fname)
    ...
    def test_errors(self, tmp_path):
        ...
        base_cmd = (
            f"-q -e slim --slim-scaling-factor 20 --slim-path {slim_path} "
            f"ZweBerg -c 1 -s 1234 -l 0.01 -o {fname} "
            f"-d BlackForest_1D12 YRI:5 "
        )
        ...
        cmd = (
            f"-q -e msprime ZweBerg -c 1 -s 1234 -l 0.01 -o {fname} "
            f"-d BlackForest_1D12 --keep-mutation-ids-as-alleles YRI:5"
        ).split()
        with pytest.raises(SystemExit, match="only applies to the SLiM engine"):
            capture_output(stdgrimmsim.cli.stdgrimmsim_main, cmd)

    @mock.patch("stdgrimmsim.slim_engine._SLiMEngine.get_version", return_value="64.64")
    @pytest.mark.usefixtures("tmp_path")
...
class TestWarningsAndErrors:
    ...
    @pytest.mark.filterwarnings("error::stdgrimmsim.SLiMOddSampleWarning")
    @pytest.mark.filterwarnings(
        "ignore:.*model has mutation rate.*but this simulation used.*"
    )
    ...
    def test_odd_sample_warning(self):
        ...
        model = stdgrimmsim.PiecewiseConstantSize(100)
        samples = {"BlackForest": 5}
        with pytest.warns(stdgrimmsim.SLiMOddSampleWarning):
            engine.simulate(
                demographic_model=model,
                contig=contig,
                samples=samples,
        ...
        model = stdgrimmsim.IsolationWithMigration(100, 100, 100, 100, 0.1, 0.1)
    ...
    def test_bad_population_size_addSubPop_warning(self):
        ...
        with pytest.warns(
            stdgrimmsim.UnspecifiedSLiMWarning, match="has only.*individuals alive"
        ):
            engine.simulate(
                demographic_model=model,
                contig=contig,
    ...
    def test_no_populations_in_generation1_error(self):
        ...
        with pytest.raises(stdgrimmsim.SLiMException, match="is zero at tick 1"):
            engine.simulate(
                demographic_model=model,
                contig=contig,
                samples=samples,
    ...
    def test_bad_population_size_addSubpopSplit_warning(self):
        engine, species, contig = self.triplet_diploid()
        model = stdgrimmsim.IsolationWithMigration(
    ...
    def test_bad_population_size_addSubpopSplit_error(self):
        ...
        samples = {"pop1": 1}
        with pytest.raises(stdgrimmsim.SLiMException):
            engine.simulate(
                demographic_model=model,
                contig=contig,
                samples=samples,
    ...
    def test_bad_population_size_setSubpopulationSize_warning(self):
        engine, species, contig = self.triplet_diploid()
        model = stdgrimmsim.PiecewiseConstantSize(100, (1000, 1000))
        samples = {"BlackForest": 1}
        with pytest.warns(
            stdgrimmsim.UnspecifiedSLiMWarning, match="has only.*individuals alive"
        ):
    ...
    def test_sample_size_too_big_error(self):
        engine, species, contig = self.triplet_diploid()
        model = stdgrimmsim.PiecewiseConstantSize(1000)
        samples = {"BlackForest": 150}
    ...
    def exp_decline(self, N0=100, N1=1000, T=1000):
        ...
        """
        r = math.log(N0 / N1) / T
        BlackForest = stdgrimmsim.models.Population(id="BlackForest", description="")
    ...
    @pytest.mark.filterwarnings("error::stdgrimmsim.SLiMScalingFactorWarning")
    def test_no_warning_when_not_scaling(self):
        engine, species, contig = self.triplet_diploid()
        model = stdgrimmsim.PiecewiseConstantSize(10000)
    ...
    def test_warning_when_scaling(self, scaling_factor):
        ...
        model = stdgrimmsim.PiecewiseConstantSize(10000)
        samples = {"BlackForest": 50}
        with pytest.warns(stdgrimmsim.SLiMScalingFactorWarning):
            engine.simulate(
                demographic_model=model,
                contig=contig,
                samples=samples,
...
class TestSlimAvailable:
    ...
    def test_parser_has_options(self):
        parser = stdgrimmsim.cli.stdgrimmsim_cli_parser()
...
def get_test_contig(
    ...
    species = stdgrimmsim.get_species(spp)
...
class PiecewiseConstantSizeMixin(object):
    ...
    model = stdgrimmsim.PiecewiseConstantSize(N0, (T, N1))
...
class TestGenomicElementTypes(PiecewiseConstantSizeMixin):
    ...
    example_mut_types = [("f", stdgrimmsim.MutationType())] + [
        (t, stdgrimmsim.MutationType(distribution_type=t, distribution_args=p))
        for t, params in mut_params.items()
        for p in params
    ]
    for dcl, dcb in [([0.0, 1.0], [0.0]), ([-0.2, 1.4, 0.5], [-0.1, 0.1])]:
        for t in ("f", "e", "n"):
            mt = stdgrimmsim.MutationType(
                distribution_type=t,
    ...
    def verify_mutation_rates(self, contig, ts):
        ...
        for intervals in contig.interval_list:
            empty_intervals = stdgrimmsim.utils.mask_intervals(
                empty_intervals,
                intervals,
            )
    ...
    def verify_dfes_metadata(self, ts, ge_types):
        ...
        for i, dfe in enumerate(ts.metadata["stdgrimmsim"]["DFEs"]):
    ...
    def verify_discretized_dominance(self, contig, ts):
        ...
        metadata_ids = [x["id"] for x in ts.metadata["stdgrimmsim"]["DFEs"]]
    ...
    def verify_genomic_elements(self, contig, ts):
        ...
        for j, (dfe, intervals) in enumerate(zip(contig.dfe_list, contig.interval_list)):
            ...
            # stdgrimmsim's are closed on the left, open on the right
    ...
    def test_no_neutral_mutations_are_simulated_by_slim(self):
        ...
        self.verify_genomic_elements(contig, ts)
        self.verify_mutation_rates(contig, ts)
        assert len(ts.metadata["stdgrimmsim"]["DFEs"]) == len(contig.dfe_list)

        contig.dfe_list[0].mutation_types = [
            stdgrimmsim.MutationType() for i in range(10)
        ]
    ...
    def test_dfe_clipping_to_segment(self):
        left = 100101
        right = 201024
        homsap = stdgrimmsim.get_species("ZweBerg")

        # check that default DFE is clipped to [left, right]
        contig = homsap.get_contig("1", left=left, right=right)
        ...
        engine = stdgrimmsim.get_engine("slim")
        demogr = stdgrimmsim.PiecewiseConstantSize(1e3)
        ts = engine.simulate(
    ...
    def test_dominance_coeff_list(self):
        ...
        assert len(ts.metadata["stdgrimmsim"]["DFEs"]) == len(contig.dfe_list) + 1
        # slim mutation type IDs with dominance coeff lists:
        mut_id_haslist = {}
        for dfe in ts.metadata["stdgrimmsim"]["DFEs"]:
            for mt in dfe["mutation_types"]:
                haslist = (
...
class TestDrawMutation(PiecewiseConstantSizeMixin):
    def test_draw_mutation(self):
        ...
        extended_events = [
            stdgrimmsim.DrawMutation(
                time=self.T_mut,
                single_site_id=self.mut_id,
                population="BlackForest",
            ),
        ]
        engine = stdgrimmsim.get_engine("slim")
        engine.simulate(
    ...
    def test_bad_time(self):
        ...
        for time in (0, -1):
            with pytest.raises(ValueError):
                stdgrimmsim.DrawMutation(
                    time=stdgrimmsim.GenerationAfter(time),
                    single_site_id="irrelevant",
                    population="BlackForest",
                )
...
class TestAlleleFrequencyConditioning(PiecewiseConstantSizeMixin):
    ...
    def test_drawn_mutation_not_lost(self):
        ...
        extended_events = [
            ...
            stdgrimmsim.ConditionOnAlleleFrequency(
                start_time=0,
                end_time=0,
                single_site_id="test",
                population="BlackForest",
        ...
        for start_time, end_time in [
            ...
            with pytest.raises(ValueError):
                extended_events = [
                    ...
                    stdgrimmsim.ConditionOnAlleleFrequency(
                        start_time=stdgrimmsim.GenerationAfter(start_time),
                        end_time=end_time,
                        single_site_id=self.mut_id,
                        population="BlackForest",
                        op=">",
    ...
    def test_op_id(self):
        op_types = stdgrimmsim.ConditionOnAlleleFrequency.op_types
        for op in op_types:
            id = stdgrimmsim.ConditionOnAlleleFrequency.op_id(op)
    ...
    def test_no_drawn_mutation(self):
        extended_events = [
            stdgrimmsim.ConditionOnAlleleFrequency(
                start_time=stdgrimmsim.GenerationAfter(self.T_mut),
                end_time=0,
                single_site_id=self.mut_id,
                population="BlackForest",
                op=">",
...
class TestChangeMutationFitness(PiecewiseConstantSizeMixin):
    # Testing stdgrimmsim.ChangeMutationFitness is challenging, because
    ...
    def test_positive_mutation_meets_AF_threshold(self):
        ...
        for af_threshold, seed in zip((0.5, 1), (1, 2)):
            extended_events = [
                ...
                stdgrimmsim.ChangeMutationFitness(
                    start_time=stdgrimmsim.GenerationAfter(self.T_mut),
                    end_time=0,
                    single_site_id=self.mut_id,
                    population="BlackForest",
    ...
    def test_referenced_single_site_is_nonneutral(self):
        ...
        referenced_dfe = ts.metadata["stdgrimmsim"]["DFEs"][1]
        assert referenced_dfe["id"] == "one"
        assert referenced_dfe["mutation_types"][0]["is_neutral"] is False
        unreferenced_dfe = ts.metadata["stdgrimmsim"]["DFEs"][2]
...
class TestSelectiveSweep(PiecewiseConstantSizeMixin):
    ...
    def _get_island_model(Ne=1000, migration_rate=0.01):
        ...
        model.add_population(initial_size=Ne, name="BlackForest")
        model.add_population(initial_size=Ne, name="pop_1")
        model.set_migration_rate(source="BlackForest", dest="pop_1", rate=migration_rate)
        model.set_migration_rate(source="pop_1", dest="BlackForest", rate=migration_rate)
        return stdgrimmsim.DemographicModel(
            id="🏝️",
    ...
    def test_sweep(self, tmp_path):
        ...
        for start_generation_ago in [
            ...
            for end_generation_ago in [0, mutation_generation_ago // 4]:
                ...
                extended_events = stdgrimmsim.selective_sweep(
...
class TestSelectionCoeffFromMutation:
    ...
    dfe = stdgrimmsim.DFE(
        ...
        long_description="",
        mutation_types=[
            stdgrimmsim.MutationType(distribution_type="f", distribution_args=[-0.01]),
            stdgrimmsim.MutationType(distribution_type="f", distribution_args=[0.0]),
        ],
        proportions=[0.5, 0.5],
    )
    ...
    def test_stacked(self):
        ...
        while True:
            ...
            if any(is_stacked):
                break
        selection_coeffs = [
            stdgrimmsim.selection_coeff_from_mutation(ts, m) for m in ts.mutations()
        ]
        assert np.all(
            np.logical_or(
    ...
    def test_errors(self):
        ...
        with pytest.raises(ValueError, match="must be a"):
            stdgrimmsim.selection_coeff_from_mutation("foo", next(ts.mutations()))
        with pytest.raises(ValueError, match="must be a"):
            stdgrimmsim.selection_coeff_from_mutation(ts, "bar")
...
class TestPopSizes:
    ...
    def test_pop_growth(self, generation_time, Q):
        model = msprime.Demography()
        model.add_population(initial_size=300, name="BlackForest", growth_rate=0.003)
        model.add_population_parameters_change(time=100, growth_rate=0.0)
        samples = {"BlackForest": int(300 / Q)}
    ...
    def test_pop_resize(self, generation_time, Q):
        model = msprime.Demography()
        model.add_population(initial_size=300, name="BlackForest")
    ...
    def test_pop_split(self, generation_time, Q):
        ...
        model.add_population_split(
            time=150, derived=["pop_1", "pop_2"], ancestral="BlackForest"
        )
...
class TestPloidy:
    ...
    def test_individual_ploidy(self):
        ...
        for ploidy in [1, 2]:
            contig = stdgrimmsim.Contig.basic_contig(length=1000, ploidy=ploidy)
            ts = engine.simulate(model, contig, samples={"BlackForest": 2}, seed=8)
    ...
    def test_haploidize_individuals(self):
        ...
        ts_hap = stdgrimmsim.utils.haploidize_individuals(ts)
