...
import stdgrimmsim
from stdgrimmsim import models
from stdgrimmsim import utils
...
class TestAllModels:
    ...
    def test_non_empty(self):
        assert len(list(stdgrimmsim.all_demographic_models())) > 0

    @pytest.mark.parametrize("model", stdgrimmsim.all_demographic_models())
...
class TestModelsEqual:
    ...
    def test_known_models(self):
        ...
        for model in stdgrimmsim.all_demographic_models():
...
class TestPopulationSampling:
    def make_model(self):
        ...
        _pop1 = stdgrimmsim.Population("pop_0", "Test pop. 0")
        _pop2 = stdgrimmsim.Population("pop_1", "Test pop. 1", sampling_time=10)
        _pop3 = stdgrimmsim.Population("pop_2", "Test pop. 2", sampling_time=None)
    ...
    def test_deprecated_get_samples(self):
        ...
        with pytest.warns(stdgrimmsim.DeprecatedFeatureWarning):
...
class TestRecombinationRates:
    ...
    def test_recombination_rate_match(self):
        ...
        contig = species.get_contig("1", recombination_rate=model.recombination_rate)
        assert model.recombination_rate == contig.recombination_map.mean_rate
