...
def gff_recarray_to_stdgrimmsim_intervals(gff):
    """
    Merge overlapping intervals and convert coordinates. GFF intervals are
    1-based [i,j], but stdgrimmsim intervals are 0-based [i-1,j).
    """
...
def get_gff_recarray(url, sha256):
    ...
    if not pathlib.Path(local_path).exists():
        ...
        stdgrimmsim.utils.download(url, local_path)

    logger.info("checking sha256")
    local_sha256 = stdgrimmsim.utils.sha256(local_path)
...
def download_process_annotations():
    ...
    """
    for spc in stdgrimmsim.all_species():
        if spc.annotations:
            for an in spc.annotations:
                ...
                for chrom_id in CHROM_IDS:
                    ...
                    if len(chrom_exons) == 0:
                        continue
                    intervals = gff_recarray_to_stdgrimmsim_intervals(chrom_exons)
                    # double check that the intervals can be used in stdgrimmsim
                    stdgrimmsim.utils._check_intervals_validity(intervals)
