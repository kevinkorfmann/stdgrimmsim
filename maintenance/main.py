...
This interface is for stdgrimmsim developers only.
"""
...
_genome = stdgrimmsim.Genome.from_data(
    ...
    citations=[
        stdgrimmsim.Citation(
            author="", year=-1, doi="", reasons={stdgrimmsim.CiteReason.ASSEMBLY}),
        stdgrimmsim.Citation(
            author="", year=-1, doi="", reasons={stdgrimmsim.CiteReason.REC_RATE}),
        stdgrimmsim.Citation(
            author="", year=-1, doi="", reasons={stdgrimmsim.CiteReason.MUT_RATE})
    ]
)
...
_species = stdgrimmsim.Species(
    ...
    citations=[
        stdgrimmsim.Citation(
            author="", year=-1, doi="", reasons={stdgrimmsim.CiteReason.POP_SIZE}),
        stdgrimmsim.Citation(
            author="", year=-1, doi="", reasons={stdgrimmsim.CiteReason.GEN_TIME})
    ],
)
...
class TestSpeciesData(test_species.SpeciesTestBase):
    ...
    species = stdgrimmsim.get_species("$sps_id")
...
class TestGenomeData(test_species.GenomeTestBase):
    ...
    genome = stdgrimmsim.get_species("$sps_id").genome
...
def ensembl_stdgrimmsim_id(ensembl_id):
...
def ncbi_stdgrimmsim_id(ncbi_id):
    tmp = ncbi_id.split(" ")[:2]
    sps_id = "".join([x[0:3].capitalize() for x in tmp])
    if len(sps_id) != 6:
...
def catalog_path(sps_id):
    return pathlib.Path(f"stdgrimmsim/catalog/{sps_id}")
...
class DataWriter:
    """
    Writes data obtained from upstream sources into the stdgrimmsim
    package hierarchy.
    """
    ...
    def add_species(self, ensembl_id, force=False):
        sps_id = ensembl_stdgrimmsim_id(ensembl_id)
    ...
    def write_ensembl_release(self):
        ...
        path = pathlib.Path("stdgrimmsim/catalog/ensembl_info.py")
...
def list_species():
    """
    List species in stdgrimmsim with their Ensembl IDs.
    """
...
def update_genome_data(species):
    ...
    if len(species) == 0:
        species_list = list(stdgrimmsim.all_species())
