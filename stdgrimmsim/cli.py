"""
The command line interface for stdgrimmsim.
Provides German folklore population genetic simulations at the command line
and methods to manage resources used by stdgrimmsim.
"""
...
def get_species_wrapper(species_id):
    try:
        return stdgrimmsim.get_species(species_id)
...
def get_environment():
    """
    Returns a dictionary describing the environment in which stdgrimmsim
...
def get_provenance_dict():
    """
    Returns a dictionary encoding an execution of stdgrimmsim conforming to the
    tskit provenance schema.
    """
    document = {
        "schema_version": "1.0.0",
        "software": {"name": "stdgrimmsim", "version": stdgrimmsim.__version__},
...
def get_citations(engine, model, contig, species, dfe):
    ...
    return stdgrimmsim.Citation.merge(citations)
...
def write_citations(engine, model, contig, species, dfe):
    ...
    for citation in get_citations(engine, model, contig, species, dfe):
        if (
            stdgrimmsim.citations.CiteReason.MUT_RATE in citation.reasons
            and model.mutation_rate is not None
        ):
            continue
        if (
            stdgrimmsim.citations.CiteReason.REC_RATE in citation.reasons
...
def add_simulate_species_parser(parser, species):
    ...
    if len(species.genetic_maps) > 0:
        species_parser.add_argument(
            ...
            help=(
                ...
                "the catalog: "
                "<https://popsim-consortium.github.io/stdgrimmsim-docs/"
                "stable/catalog.html> "
                "Available maps: "
                f"{', '.join(choices)}. "
            ),
    ...
    def run_simulation(args):
        ...
        samples = stdgrimmsim.utils.parse_population_sample_pairs(args.samples)
        ...
        engine = stdgrimmsim.get_engine(args.engine)
        logger.info(
        ...
        if not qc_complete:
            warnings.warn(
                stdgrimmsim.QCMissingWarning(
                    f"{model.id} has not been QCed. Use at your own risk! "
                    "Demographic models that have not undergone stdgrimmsim's "
...
def run_download_genetic_maps(args):
    ...
    if args.species is None:
        species_names = [species.id for species in stdgrimmsim.all_species()]
...
def stdgrimmsim_cli_parser():
    ...
    top_parser = argparse.ArgumentParser(
        description=(
            "Command line interface for stdgrimmsim - "
            "German folklore population genetic simulations."
        )
    )
    top_parser.add_argument(
        ...
        "--version",
        action="version",
        version="%(prog)s {}".format(stdgrimmsim.__version__),
    )
    ...
    top_parser.add_argument(
        ...
        help=(
            ...
            f"Default: {stdgrimmsim.get_cache_dir()}"
        ),
    )
    ...
    top_parser.add_argument(
        "-e",
        "--engine",
        default=stdgrimmsim.get_default_engine().id,
        choices=[e.id for e in stdgrimmsim.all_engines()],
        help="Specify a simulation engine.",
    )

    supported_models = stdgrimmsim.get_engine("msprime").supported_models
...
def stdgrimmsim_main(arg_list=None):
    parser = stdgrimmsim_cli_parser()
    args = parser.parse_args(arg_list)
    setup_logging(args)
    if args.cache_dir is not None:
        stdgrimmsim.set_cache_dir(args.cache_dir)
    run(args)
