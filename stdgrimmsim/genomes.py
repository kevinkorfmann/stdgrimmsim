...
class Contig:
    ...
    .. note::
        To run stdgrimmsim simulations with alternative, user-specified mutation,
    ...
    def __attrs_post_init__(self):
        ...
        if self.species is None:
            self.species = stdgrimmsim.species.Species(
                id="default",
    ...
    def species_contig(
        *,
        ...
        if chromosome is not None and chromosome.lower() in non_autosomal_lower:
            warnings.warn(
                stdgrimmsim.NonAutosomalWarning(
                    "Non-autosomal simulations are not yet supported. See "
                    "https://github.com/popsim-consortium/stdgrimmsim/issues/383 and "
                    "https://github.com/popsim-consortium/stdgrimmsim/issues/406"
                )
            )
        if (
        ...
        if length_multiplier is None:
            ...
            warnings.warn(
                stdgrimmsim.DeprecatedFeatureWarning(
        ...
        if chromosome is None:
            ...
            if inclusion_mask is not None:
                ...
                if isinstance(inclusion_mask, str):
                    inclusion_intervals = stdgrimmsim.utils.read_bed(
                        inclusion_mask, chromosome
                    )
                else:
                    inclusion_intervals = inclusion_mask
                inclusion_intervals = stdgrimmsim.utils.clip_intervals(
            ...
            if exclusion_mask is not None:
                ...
                if isinstance(exclusion_mask, str):
                    exclusion_intervals = stdgrimmsim.utils.read_bed(
                        exclusion_mask, chromosome
                    )
                else:
                    exclusion_intervals = exclusion_mask
                exclusion_intervals = stdgrimmsim.utils.clip_intervals(
    ...
    def add_dfe(self, intervals, DFE):
        ...
        """
        _, left, right = self.coordinates
        intervals = stdgrimmsim.utils.clip_intervals(intervals, left, right)
        stdgrimmsim.utils._check_intervals_validity(intervals, start=left, end=right)
        for j, ints in enumerate(self.interval_list):
            self.interval_list[j] = stdgrimmsim.utils.mask_intervals(ints, intervals)
    ...
    def add_single_site(
        ...
        mut_type = stdgrimmsim.MutationType(
            distribution_type="f",
            dominance_coeff=0.5,
