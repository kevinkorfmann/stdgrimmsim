...
1 early() {
    ...
    // For each stdgrimmsim mutation type with an h-s relationship
...
_raw_stdgrimmsim_top_level_schema = {
    "stdgrimmsim": {
...
def _add_dfes_to_metadata(ts, contig):
    ...
    schema["properties"].update(_raw_stdgrimmsim_top_level_schema)
    ...
    new_metadata = {"stdgrimmsim": {"DFEs": dfes}}
...
def slim_makescript(
    ...
    def fix_time(event):
        for attr in ("time", "start_time", "end_time"):
            ...
            if isinstance(t, stdgrimmsim.GenerationAfter):
    ...
    op_id = stdgrimmsim.ConditionOnAlleleFrequency.op_id
    ...
    for ee in extended_events:
        ...
        if hasattr(ee, "start_time") and hasattr(ee, "end_time"):
            ...
            stdgrimmsim.validate_time_range(ee.start_time, ee.end_time)
        ...
        if isinstance(ee, stdgrimmsim.DrawMutation):
        ...
        elif isinstance(ee, stdgrimmsim.ChangeMutationFitness):
        ...
        elif isinstance(ee, stdgrimmsim.ConditionOnAlleleFrequency):
            assert population_id >= 0
    ...
    # Header
    printsc("/*")
    printsc(" * stdgrimmsim " + stdgrimmsim.__version__)
    printsc(" *")
    ...
    for j, d, ints in _enum_dfe_and_intervals(contig):
        ...
        if ints.shape[0] > 0:
            ...
            # stdgrimmsim intervals are 0-based left inclusive, right exclusive, but
    ...
    op_types = ", ".join(
        f'"{op}"' for op in stdgrimmsim.ConditionOnAlleleFrequency.op_types
    )
    ...
    for sample_set in samples:
        ...
        if sample_set.ploidy == 1:
            ...
            if count % 2 != 0:
                ...
                warnings.warn(
                    stdgrimmsim.SLiMOddSampleWarning(
...
class _SLiMEngine(stdgrimmsim.Engine):
    id = "slim"  #:
    ...
    citations = [
        stdgrimmsim.Citation(
            ...
            year=2019,
            author="Haller et al.",
            reasons={stdgrimmsim.CiteReason.ENGINE},
        ),
    ]
    ...
    def simulate(
        ...
        * ``separate_sexes``: if the species has separate sexes, SLiM
        ...
        if slim_scaling_factor != 1:
            warnings.warn(
                stdgrimmsim.SLiMScalingFactorWarning(
        ...
        def _slim_tempdir():
            tempdir = tempfile.TemporaryDirectory(
                prefix="stdgrimmsim_", ignore_cleanup_errors=True
            )
    ...
    def _run_slim(
        ...
        with subprocess.Popen(
            ...
            try:
                ...
                warnings.warn(
                    stdgrimmsim.UnspecifiedSLiMWarning(
            ...
            for line in outs.splitlines():
                ...
                if line.startswith("WARNING: "):
                    warnings.warn(
                        stdgrimmsim.UnspecifiedSLiMWarning(line[len("WARNING: ") :])
                    )
                else:
    ...
    def _recap_and_rescale(
        ...
        if extended_events is not None:
            ...
            for dfe in metadata["stdgrimmsim"]["DFEs"]:
        ...
        metadata["stdgrimmsim"]["DFEs"].append(recap_dfe)
        tables.metadata = metadata
        ...
        if contig.ploidy == 1:
            ts = stdgrimmsim.utils.haploidize_individuals(ts)
    ...
    def recap_and_rescale(
        ...
        **kwargs,
        ...
        run outside of stdgrimmsim, this function can be used to transform the
        SLiM tree sequence following the procedure that would have been used
        if stdgrimmsim had run SLiM itself.
        ...
        .. warning::
            ...
            make to the SLiM code before using it, the stdgrimmsim source code
...
stdgrimmsim.register_engine(_SLiMEngine())
