import stdgrimmsim
from . import genome_data

_citation = stdgrimmsim.Citation(
    author="Saxon / Erzgebirge folklore",
    year=1700,
    doi="https://en.wikipedia.org/wiki/Ore_Mountains",
    reasons={stdgrimmsim.CiteReason.GEN_TIME, stdgrimmsim.CiteReason.POP_SIZE},
)
_assembly_citation = stdgrimmsim.Citation(
    author="Saxon folklore",
    year=1700,
    doi="https://en.wikipedia.org/wiki/Ore_Mountains",
    reasons={stdgrimmsim.CiteReason.ASSEMBLY},
)
_mutation_citation = stdgrimmsim.Citation(
    author="Saxon folklore",
    year=1700,
    doi="https://en.wikipedia.org/wiki/Ore_Mountains",
    reasons={stdgrimmsim.CiteReason.MUT_RATE, stdgrimmsim.CiteReason.REC_RATE},
)

_recombination_rate = {c: 5.5e-8 for c in genome_data.data["chromosomes"]}
_recombination_rate["mine_mitogenome"] = 0
_mutation_rate = {c: 2e-8 for c in genome_data.data["chromosomes"]}
_mutation_rate["mine_mitogenome"] = 7.2e-8
_species_ploidy = 2
_ploidy = {c: _species_ploidy for c in genome_data.data["chromosomes"]}
_ploidy["mine_mitogenome"] = 1

_genome = stdgrimmsim.Genome.from_data(
    genome_data.data,
    recombination_rate=_recombination_rate,
    mutation_rate=_mutation_rate,
    ploidy=_ploidy,
    citations=[_mutation_citation, _assembly_citation],
)

_species = stdgrimmsim.Species(
    id="SaxErz",
    ensembl_id="bergmann_erzgebirgensis",
    name="Bergmann erzgebirgensis",
    common_name="Erzgebirge spirit (Ore Mountain / Bergmännlein)",
    separate_sexes=True,
    genome=_genome,
    generation_time=28,
    population_size=72_000,
    ploidy=_species_ploidy,
    citations=[_citation],
)
stdgrimmsim.register_species(_species)
