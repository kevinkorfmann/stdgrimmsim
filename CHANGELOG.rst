...
- To add the possibility for a relationship between dominance and selection
  coefficient, now each stdgrimmsim MutationType might have more than one
  underlying SLiM mutation type; so, where this is recorded in top-level
  metadata (under `ts.metadata["stdgrimmsim"]["DFEs"]`) is now a list
...
- SLiM extended events and selective sweep infrastructure have been
  moved from the `stdgrimmsim.ext` namespace into ``stdgrimmsim`` proper.
...
- Sample specification has switched from positional and haploid (e.g.
  ``stdgrimmsim HomSap -d OutOfAfrica_3G09 6 0 10``) to named with species-specific
  ploidy (equivalent to ``stdgrimmsim HomSap -d OutOfAfrica_3G09 YRI:3 CEU:0
...
- Move the --quiet/-q command line option to the top-level. Previously
  we would write ``stdgrimmsim HomSap -q 10`` whereas we now write
  ``stdgrimmsim -q HomSap``. (:user:`jeromekelleher`, :issue:`515`, :pr:`547`)
