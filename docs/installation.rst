...
You can get started right away, without installing ``stdgrimmsim`` locally, by using the
`Jupyter Binder <https://mybinder.org/v2/gh/kevinkorfmann/stdgrimmsim/main?filepath=stdgrimmsim_example.ipynb>`_. |binder|

.. |binder| image:: https://mybinder.org/badge_logo.svg
 :target: https://mybinder.org/v2/gh/kevinkorfmann/stdgrimmsim/main?filepath=stdgrimmsim_example.ipynb


There are two basic options for installing ``stdgrimmsim`` and its dependencies: either
...
Library requirements for ``stdgrimmsim`` should be installed automatically when using
either conda or pip.

``stdgrimmsim`` requires Python 3.10 or later.
...
Pre-built binary packages for ``stdgrimmsim`` are available through
...
using::
    ...
    $ conda install -c conda-forge stdgrimmsim
...
3. Install stdgrimmsim: ``conda install stdgrimmsim``.
4. Try it out: ``stdgrimmsim --version``.
...
Installing using pip is usually as simple as::
    ...
    $ python -m pip install stdgrimmsim --user

This will install ``stdgrimmsim`` into your local user Python packages
