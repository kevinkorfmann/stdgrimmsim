...
``stdgrimmsim`` **is a community effort, and we welcome YOU to join us!**

We envision at least three main types of ``stdgrimmsim`` developers:
...
contributors to learn the ins and outs of ``stdgrimmsim`` development.
...
``stdgrimmsim``, establishment of benchmarking pipelines,
...
GitHub `issue <http://github.com/kevinkorfmann/stdgrimmsim/issues>`__., or adding or editing
the documentation. See the section `Documentation`_.

Get into contact with the ``stdgrimmsim`` community by subscribing to our
`email list serve <https://lists.uoregon.edu/mailman/listinfo/popgen_benchmark>`__
and by creating and commenting on
Github `issue <http://github.com/kevinkorfmann/stdgrimmsim/issues>`__.
There is a lot of chatter through
`Github <http://github.com/kevinkorfmann/stdgrimmsim>`__, and we've been building code
there cooperatively.
If you want to help out and don't know where to start, you can look through the
list of
`Good first issues
<https://github.com/kevinkorfmann/stdgrimmsim/issues?q=is%3Aopen+is%3Aissue+label%3A%22
...
To get started helping with ``stdgrimmsim`` development, please read the
following sections to learn how to contribute.
And, importantly, please have a look at our
`code of conduct <https://github.com/kevinkorfmann/stdgrimmsim/blob/main/CODE_OF_CONDUCT.md>`__.
...
The ``stdgrimmsim`` library requires Python 3.4 or later.

For ``pip`` users, install the packages required for development using::

    $ python3 -m pip install -r requirements/development.txt

You can then install the development version of ``stdgrimmsim`` like this::
...
First, create the virtual environment (You only need to do this once)::
    ...
    $ python3 -m venv stdgrimmsim_env

Next, activate the virtual environment::

    $ source stdgrimmsim_env/bin/activate
...
Once the virtual environment is activated, install the requirements::
    ...
    (stdgrimmsim_env) $ python3 -m pip install -r requirements/development.txt
...
***************
    ...
    1. Make your own `fork <https://help.github.com/articles/fork-a-repo/>`__
       of the ``stdgrimmsim`` repository on GitHub, and
...
look at the `Network <https://github.com/kevinkorfmann/stdgrimmsim/network>`__ view on
...
.. code-block:: none
    ...
    To github.com:jeromekelleher/stdgrimmsim.git
    ! [rejected]        topic_branch_name -> topic_branch_name (non-fast-forward)
    error: failed to push some refs to 'git@github.com:jeromekelleher/stdgrimmsim.git'
...
In ``stdgrimmsim``, we aim to be inclusive and facilitate adding a diverse range of species.
...
but you still think it will be useful to add it to ``stdgrimmsim``,
then we encourage you to `open an issue <http://github.com/kevinkorfmann/stdgrimmsim/issues/new>`__
...
Every species added to ``stdgrimmsim`` should have the following information available:
...
1. The **genome assembly** should consist of a list of chromosomes or scaffolds and their lengths.
   ...
   is essential for chromosome-level simulations produced by ``stdgrimmsim``.
   Thus, currently, ``stdgrimmsim`` only supports adding species with near-complete
...
Before you add your species to ``stdgrimmsim``, see that you can collect the values
...
feel free to `open an issue <http://github.com/kevinkorfmann/stdgrimmsim/issues/new>`__
on the GitHub repository to get assistance from other members of the ``stdgrimmsim`` community.
...
If this is your first time adding a species to ``stdgrimmsim``, it's a good
...
don't hesitate to
`open a new issue <https://github.com/kevinkorfmann/stdgrimmsim/issues/new>`__.
We're more than happy to answer any questions and help get you up and running.
Before you add any code, be sure to have forked the ``stdgrimmsim`` repository
...
``stdgrimmsim/catalog/DroMel/`` in the repository.

Once the species directory is set up, you may use the ``maintenance`` utility
of ``stdgrimmsim`` to generate template files where you can enter
...
(``stdgrimmsim/catalog/<SPECIES_ID>/``):
...
``stdgrimmsim.Citation`` object indicating the publication from which it was derived.
Each ``stdgrimmsim.Citation`` object is initialized with the following information:
...
* reasons (list of ``stdgrimmsim.CiteReason``):
  possible reasons to include a citation in ``species.py`` are:

  * ``stdgrimmsim.CiteReason.ASSEMBLY``
  * ``stdgrimmsim.CiteReason.REC_RATE``
  * ``stdgrimmsim.CiteReason.MUT_RATE``
  * ``stdgrimmsim.CiteReason.POP_SIZE``
  * ``stdgrimmsim.CiteReason.GEN_TIME``

To demonstrate how the ``_genome`` and ``_species`` objects are set,
we provide below a detailed example for *A. thaliana*
(see also ``stdgrimmsim/catalog/AraTha/species.py``).
...
The ``_genome`` object is then defined by calling the ``stdgrimmsim`` function
``stdgrimmsim.Genome.from_data``.
...
and a list of ``stdgrimmsim.Citation`` objects.
  ...
  _genome = stdgrimmsim.Genome.from_data(
      ...
      citations=[
          stdgrimmsim.Citation(
              ...
              reasons={stdgrimmsim.CiteReason.MUT_RATE},
          ),
          stdgrimmsim.Citation(
              author="Huber et al.",
              year=2014,
              doi="https://doi.org/10.1093/molbev/msu247",
              reasons={stdgrimmsim.CiteReason.REC_RATE},
          ),
          stdgrimmsim.Citation(
...
accompanied by the appropriate ``stdgrimmsim.Citation`` objects.
    ...
    _species = stdgrimmsim.Species(
        ...
        citations=[
            stdgrimmsim.Citation(
                ...
                year=2002,
                author="Donohue",
                reasons={stdgrimmsim.CiteReason.GEN_TIME},
            ),
            stdgrimmsim.Citation(
                doi="https://doi.org/10.1016/j.cell.2016.05.063",
                year=2016,
                author="1001GenomesConsortium",
                reasons={stdgrimmsim.CiteReason.POP_SIZE},
            ),
        ],
    )
...
you should be able to load and simulate the newly added species using ``stdgrimmsim``.
...
in the species directory (``stdgrimmsim/catalog/<SPECIES_ID>/``)
...
**You have officially joined the** ``stdgrimmsim`` **development team. Welcome!!**
Your code still needs to undergo review by another member (or members)
of the development team before it is fully incorporated into ``stdgrimmsim``.
This will likely require additional feedback from you,
so, stay tuned for discussion during the review process.

----------------------------------------
Overview of the stdgrimmsim review process
----------------------------------------

We provide here a general outline for the review process we use in ``stdgrimmsim``,
...
1. After the original contributor submitted a PR with their new code,
   ...
   ``stdgrimmsim`` for basic problems or style issues.
...
2. The original contributor then opens a new **QC issue** on GitHub
   ...
   <https://github.com/kevinkorfmann/stdgrimmsim/issues/new/choose>`__
   we provide.
   ...
   <https://github.com/kevinkorfmann/stdgrimmsim/issues/new>`__
...
3. A different member of the ``stdgrimmsim`` community volunteers to review the
   ...
   **without looking at the original contributor's code**.
   ...
   added to ``stdgrimmsim`` or when you are reviewing a **demographic model**
...
7. Once the **review PR** submitted in Step 5 passes all unit tests,
   ...
   **The new code is now officially added to the** ``stdgrimmsim`` **catalog!**
...
Step 6 of the `Overview of the stdgrimmsim review process`_.
...
-----------------------------------
Any model added to ``stdgrimmsim`` should be based the **published literature**
...
The ``stdgrimmsim`` :ref:`sec_catalog` also contains a collection of **generic models**,
...
then we suggest that you `open a new issue <http://github.com/kevinkorfmann/stdgrimmsim/issues>`__
...
If this is your first time implementing a demographic model in ``stdgrimmsim``, it's a good
idea to take some time browsing the :ref:`sec_catalog`
...
don't hesitate to `open a new issue <http://github.com/kevinkorfmann/stdgrimmsim/issues>`__.
We're more than happy to answer any questions and help get you up and running.
Before you add any code, be sure to have forked the ``stdgrimmsim`` repository
and cloned it locally, following the instructions in the `GitHub Workflow`_ section.
...
file in that species directory ``stdgrimmsim/catalog/<SPECIES_ID>/``
...
then you should add this file to ``stdgrimmsim/catalog/<SPECIES_ID>/``,
...
.. code-block:: python
  ...
  _species = stdgrimmsim.get_species("<SPECIES_ID>")
...
a ``stdgrimmsim.DemographicModel`` object initialized with the appropriate values.
...
* ``populations``: A list of ``stdgrimmsim.Population`` objects, which have their own
  ``id`` and ``description``. For example, the Thousand Genomes Project Yoruba panel
  could be defined as ``stdgrimmsim.Population(id="YRI", description="1000 Genomes YRI
  (Yorubans)")``.

* ``citations``: A list of ``stdgrimmsim.Citation`` objects for the publications
  ...
  ``stdgrimmsim.CiteReason.DEM_MODEL``.
...
you should test it locally using the development version of ``stdgrimmsim``.
First, make sure to install the development version of ``stdgrimmsim`` and its requirements,
...
.. note::
    ...
    `GitHub issues <https://github.com/kevinkorfmann/stdgrimmsim/issues?q=is%3Aissue+adding+demographic+model+>`__
    or `open a new issue <https://github.com/kevinkorfmann/stdgrimmsim/issues/new>`__ to get help from others.
...
is officially added to ``stdgrimmsim`` (see `Overview of the stdgrimmsim review process`_).
In Step 3 of this process, the reviewer writes testing code for the newly
added demographic model.
This is done in file ``stdgrimmsim/qc/<SPECIES_ID>.py``
(where ``<SPECIES_ID>`` is the six-character identifier of the species).
If this is the first demographic model added for this species,
the reviewer should create this file and add an import
statement for the species to ``stdgrimmsim/qc/__init__.py``.

The code written by the reviewer in ``stdgrimmsim/qc/<SPECIES_ID>.py``
should define a function that returns a
``stdgrimmsim.DemographicModel`` object, parallel to the function defined
...
original function by adding this bit of code to ``stdgrimmsim/qc/<SPECIES_ID>.py``:
...
.. note::
    ...
    In the ``stdgrimmsim`` code and documentation, we use the term
...
`stdgrimmsim` by creating a new `GeneticMap` object and providing a formatted file
detailing recombination rates to a designated ``stdgrimmsim`` maintainer who then uploads
...
tarball). This file will be sent to one of the ``stdgrimmsim`` uploaders for placement in the
AWS cloud once the new genetic map(s) are approved. Finally, you must add a `GeneticMap`
object to the file named for your species in the ``stdgrimmsim/catalog/<SPECIES_ID>/`` directory
(the one that contains all the simulation code for that species,
see `Getting set up to add a new species`_):

.. code-block:: python

    _genetic_map_citation = stdgrimmsim.Citation(
        doi="FILL_ME", author="FILL_ME", year=9999, reasons={stdgrimmsim.CiteReason.GEN_MAP}
    )
    """
...
.. code-block:: sh
   ...
   python -c 'from stdgrimmsim.utils import sha256; print(sha256("genetic_map.tgz"))'
...
then users of previous versions of stdgrimmsim who try to use that annotation
...
current assembly is updated in ``stdgrimmsim``. This process can be partially automated by running
...
This file will be sent to one of the ``stdgrimmsim`` uploaders for placement in the
AWS cloud, once the new map is approved. Finally, you must add a `GeneticMap`
object to the file named for your species in the `stdgrimmsim/catalog/<SPECIES_ID>/`
...
This is a central component of the way that ``stdgrimmsim``
...
If this is your first time implementing a DFE in ``stdgrimmsim``, it's a good
idea to take some time browsing the :ref:`sec_catalog`
...
by defining a specialized function, which essentially returns
a ``stdgrimmsim.DFE`` object initialized with the appropriate values.
...
* ``citations``: A list of ``stdgrimmsim.Citation`` objects for the publications
  ...
  a specified reason for citing this model (see `Coding the species parameters`_).
  The reason associated with demographic model citations will typically be
  ``stdgrimmsim.CiteReason.DFE``.

* ``mutation_types``: A list of ``stdgrimmsim.MutationType`` objects corresponding to different
  mutation types (such as negative, neutral, or positive).
  For more details, see the example below and the documentation of :class:`stdgrimmsim.MutationType`
...
.. code-block:: python
    ...
    def _dfe_func_name():
        ...
        positive = stdgrimmsim.MutationType(
            dominance_coeff=0.5,
...
the model was successfully loaded to ``stdgrimmsim``.
You may follow the process outlined for `Testing your demographic model and submitting a PR`_.

Once you are convinced that the model was accurately implemented and loaded to ``stdgrimmsim``,
you should submit a pull request (PR) with your changes to the code.
See the `GitHub workflow`_ for more details about this process.
...
`new issue <https://github.com/kevinkorfmann/stdgrimmsim/issues/new>`__,
...
as for reviewing a demographic model (see `Overview of the stdgrimmsim review process`_).
...
To do this, you add your implementation to ``stdgrimmsim/qc/<SPECIES_ID>.py``,
...
To ensure that the code in ``stdgrimmsim`` is as readable as possible
...
To ensure uniformity in naming schemes across objects in ``stdgrimmsim``
...
All code added to ``stdgrimmsim`` should have
...
Test coverage is checked using `CodeCov <https://codecov.io/gh/kevinkorfmann/stdgrimmsim>`__,
...
To run the test simply use::
    ...
    $ flake8 --max-line-length 89 stdgrimmsim tests

If you would like to automatically run this test before a commit is permitted,
add the following line in the file ``stdgrimmsim/.git/hooks/pre-commit.sample``::

    exec flake8 --max-line-length 89 setup.py stdgrimmsim tests
...
`pytest` for coverage reports. This can be done with::
    ...
    $ pytest --cov-report html --cov=stdgrimmsim tests/

this will output a directory of html files for you to browse test coverage
for every file in ``stdgrimmsim`` in a reasonably straightfoward graphical
...
HTML output is written to ``docs/_build/html/``. You will need ``stdgrimmsim`` installed
...
5. After the release, if everything looks OK,
   ...
   `stdgrimmsim-docs <https://github.com/kevinkorfmann/stdgrimmsim-docs>`__
   repository
