...
There are two main ways of accessing the resources of the ``stdgrimmsim`` package
...
models in the ``stdgrimmsim`` :ref:`Catalog <sec_catalog>`. The other way to
access the ``stdgrimmsim`` resources is via the Python API. This is a bit more
complicated, but allows for more advanced tasks. This tutorial will walk
through both ways of using the ``stdgrimmsim`` package as well as a few examples
of producing and processing output.

To simulate genomes using ``stdgrimmsim``,
...
*******************************************************
Running stdgrimmsim with the command-line interface (CLI)
*******************************************************

In order to use the ``stdgrimmsim`` CLI the ``stdgrimmsim`` package must be
...
.. command-output:: stdgrimmsim --help
    :ellipsis: 30

This shows the species currently supported by ``stdgrimmsim``. This means that
``stdgrimmsim`` knows various traits of these species, including chromosome size,
recombination rate(s), etc. Once we've selected a species, in this case humans, we
can look at the help again as follows.

.. command-output:: stdgrimmsim HomSap --help
    :ellipsis: 20
...
using the species' defaults as seen in the species help (``stdgrimmsim HomSap --help``).
...
.. code-block:: console
    ...
    $ stdgrimmsim HomSap -c chr22 -o foo.ts pop_0:1

.. warning:: It's important to remember to either redirect the output of ``stdgrimmsim``
...
.. command-output:: stdgrimmsim HomSap --help-models
    :ellipsis: 30
...
.. code-block:: console
    ...
    $ stdgrimmsim HomSap -c chr22 --left 10000000 --right 20000000 -o foo.ts -d OutOfAfrica_2T12 AFR:2 EUR:3
...
.. command-output:: stdgrimmsim HomSap --help-genetic-maps
    :ellipsis: 20
...
.. code-block:: console
    ...
    $ stdgrimmsim HomSap -g HapMapII_GRCh38 -c chr22 -o foo.ts -d OutOfAfrica_2T12 AFR:2 EUR:3
...
.. code-block:: console
    ...
    $ stdgrimmsim HomSap -s 1046 -g HapMapII_GRCh38 -c chr22 -o foo.ts \
...
The output from a ``stdgrimmsim`` simulation is a *tree sequence*,
...
.. code-block:: console
    ...
    $ stdgrimmsim -e slim HomSap -c chr22 --left 10000000 --right 20000000 \
...
.. code-block:: console
    ...
    $ stdgrimmsim -e slim --slim-scaling-factor 10 HomSap -c chr22 \
...
.. code-block:: console
    ...
    $ stdgrimmsim -e slim --slim-scaling-factor 20 HomSap -c chr22 \
...
`Gamma_K17 <https://popsim-consortium.github.io/stdgrimmsim-docs/main/catalog.html#sec_catalog_homsap_dfes_gamma_k17>`_.
...
See also the Python API to incorporate `selection <https://popsim-consortium.github.io/stdgrimmsim-docs/latest/tutorial.html#incorporating-selection>`__.
...
.. code-block:: console
   ...
   $ stdgrimmsim -e slim --slim-scaling-factor 1000 DroMel -c chr2L \
...
.. code-block:: console
   ...
   $ stdgrimmsim -vv -e slim --slim-scaling-factor 1000 DroMel -c chr2L \
   $    --right 1000000 -o foo.ts -d African3Epoch_1S16 AFR:50 \
   $    | grep "^DEBUG:"
...
*************************************************
Running stdgrimmsim with the Python interface (API)
*************************************************

Nearly all the functionality of ``stdgrimmsim`` is available through the CLI,
...
with tree sequences.
In order to use the ``stdgrimmsim`` API the ``stdgrimmsim`` package must be
...
model to simulated under. In ``stdgrimmsim`` there are two types of model: ones
...
.. code-block:: python
   ...
   species = stdgrimmsim.get_species("HomSap")
...
default mutation rate for humans in the ``stdgrimmsim`` catalog. As such,
...
However, ``stdgrimmsim`` also has the ability to produce
...
The ``stdgrimmsim`` tool is designed so that different simulation engines
...
is that you can ask ``stdgrimmsim`` to output the ``SLiM`` model code directly,
without actually running the model.
You could then edit the code, to add other features not implemented in stdgrimmsim.
...
to the tree sequence that ``stdgrimmsim`` usually does.
...
but ``stdgrimmsim`` throws away all the information that is extraneous
    ...
    for site in ts.sites():
        ...
        for mut in site.mutations:
            selection_coeffs[region].append(
                stdgrimmsim.selection_coeff_from_mutation(ts, mut)
            )
...
.. code-block:: python
    ...
    species = stdgrimmsim.get_species("DroMel")
    model = stdgrimmsim.PiecewiseConstantSize(100000)
    samples = {"pop_0": 50}
...
This is done through :func:`stdgrimmsim.selective_sweep`, which represents a
...
.. code-block:: python
    ...
    homsap = stdgrimmsim.get_species("HomSap")
...
.. code-block:: python
    ...
    deepones = stdgrimmsim.get_species("DagHyd")
...
*******************************
Example analyses with stdgrimmsim
*******************************
...
.. command-output:: stdgrimmsim HomSap -c chr1 -o afr-america-chr1.trees -s 13 -g HapMapII_GRCh38 -d AmericanAdmixture_4B18 AFR:2 EUR:2 ASIA:2 ADMIX:2 -D
    :ellipsis: 18
...
.. code-block:: console
    ...
    $ stdgrimmsim HomSap -c chr1 -o afr-america-chr1.trees -s 13 -g HapMapII_GRCh38 \
...
Our work with ``stdgrimmsim`` is done; we'll now switch to a Python console and import
...
(shown in the output of ``stdgrimmsim``, or found in python with ``model.mutation_rate``),
...
This time, we will use the :meth:`stdgrimmsim.IsolationWithMigration` model.
...
We begin by importing ``stdgrimmsim`` into a Python environment and specifying our desired
...
.. code-block:: python
    ...
    species = stdgrimmsim.get_species("AraTha")
...
From the API description, we can see that the :meth:`stdgrimmsim.IsolationWithMigration`
