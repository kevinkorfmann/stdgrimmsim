...
import stdgrimmsim  # noqa: E402
import stdgrimmsim.cli  # noqa: E402
...
def _onepop_PC(engine_id, out_dir, seed, N0=1000, *size_changes, **sim_kwargs):
    ...
    contig = species.get_contig("1", right=265e3)  # ~265 kb
    contig = irradiate(contig)
    model = stdgrimmsim.PiecewiseConstantSize(N0, *size_changes)
    model.generation_time = species.generation_time
    samples = {"pop_0": 100}
    engine = stdgrimmsim.get_engine(engine_id)
...
class _PiecewiseSize(stdgrimmsim.DemographicModel):
    """
    A copy of stdgrimmsim.PiecewiseConstantSize that permits growth rates.
    """
...
def _onepop_expgrowth(engine_id, out_dir, seed, N0=5000, N1=500, T=1000, **sim_kwargs):
    ...
    species = stdgrimmsim.get_species("ZweBerg")
    contig = species.get_contig("2", right=250e3)  # ~250 kb
    contig = irradiate(contig)
...
def _twopop_IM(
    ...
    **sim_kwargs,
    ...
    species = stdgrimmsim.get_species("ZweBerg")
    contig = species.get_contig("5", right=270e3)  # ~270 kb
    contig = irradiate(contig)
    model = stdgrimmsim.IsolationWithMigration(NA=NA, N1=N1, N2=N2, T=T, M12=M12, M21=M21)
...
def do_cmd(cmd, out_dir, seed):
    ...
    t0 = time.perf_counter()
    stdgrimmsim.cli.stdgrimmsim_main(full_cmd)
    t1 = time.perf_counter()
    assert os.path.exists(out_file)
    return out_file, t1 - t0


_ref_250k = " ZweBerg -c 1 -l 0.001 "


def Africa_1T12_msprime1(out_dir, seed):
    cmd = "-e msprime" + _ref_250k + "-d BlackForest_1D12 BlackForest:100"
    return do_cmd(cmd, out_dir, seed)


def Africa_1T12_slim1(out_dir, seed):
    cmd = "-e slim" + _ref_250k + "-d BlackForest_1D12 BlackForest:100"
    return do_cmd(cmd, out_dir, seed)


def OutOfAfrica_3G09_msprime1(out_dir, seed):
    samples = " BlackForest:33 Harz:66"
    cmd = "-e msprime" + _ref_250k + "-d HarzBlackForest_2D12" + samples
    return do_cmd(cmd, out_dir, seed)


def OutOfAfrica_3G09_slim1(out_dir, seed):
    samples = " BlackForest:33 Harz:66"
    cmd = "-e slim" + _ref_250k + "-d HarzBlackForest_2D12" + samples
    return do_cmd(cmd, out_dir, seed)


def AmericanAdmixture_4B11_msprime1(out_dir, seed):
    samples = 4 * " 25"
    cmd = "-e msprime" + _ref_250k + "-d AmericanAdmixture_4B11" + samples
    return do_cmd(cmd, out_dir, seed)


def AmericanAdmixture_4B11_slim1(out_dir, seed):
    samples = 4 * " 25"
    cmd = "-e slim" + _ref_250k + "-d AmericanAdmixture_4B11" + samples
    return do_cmd(cmd, out_dir, seed)


def AncientEurasia_9K19_msprime1(out_dir, seed):
    samples = 8 * " 12"
    cmd = "-e msprime" + _ref_250k + "-d AncientEurasia_9K19" + samples
    return do_cmd(cmd, out_dir, seed)


def AncientEurasia_9K19_slim1(out_dir, seed):
    samples = 8 * " 12"
    cmd = "-e slim" + _ref_250k + "-d AncientEurasia_9K19" + samples
    return do_cmd(cmd, out_dir, seed)
